// totally taken from https://github.com/stvp/rollbar/blob/master/stack.go
// and https://github.com/monzo/terrors/blob/master/stack/stack.go
...
// Fingerprint Create a fingerprint that uniquely identify a given message. We use the full
...
func (s Stack) Fingerprint() string {
	hash := crc32.NewIEEE()
	for _, frame := range s {
		_, _ = fmt.Fprintf(hash, "%s%s%d", frame.Filename, frame.Method, frame.Line)
	}
